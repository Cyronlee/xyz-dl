...
async function parsePodcastPage(podcastUrl) {
  ...
  console.log(`⚠️  注意：小宇宙页面为动态加载，可能无法直接解析`);
  console.log(`💡 建议使用链接列表文件方式：xyz-dl batch episodes.txt -a download\n`);
  ...
  const episodes = [];
  const podcastName = $('h1').first().text().trim() || $('title').text().replace('小宇宙', '').trim() || '未知播客';
